...
<head>
  <meta charset="utf-8">
  <title>Communauté · Studimove</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .coming-hero{margin:32px 18px 0;background:linear-gradient(135deg,var(--soft) 0%,#e0f0ff 100%);border-radius:28px;padding:36px 24px;text-align:center;border:1px solid #dbeafe}
    .coming-icon{font-size:64px;margin-bottom:16px}
    .coming-title{font-size:24px;font-weight:950;letter-spacing:-.5px;margin:0 0 10px;color:#111}
    .coming-sub{font-size:14px;color:#667085;font-weight:700;line-height:1.6;margin:0 0 24px}
    .coming-badge{display:inline-block;background:linear-gradient(90deg,var(--blue),var(--cyan));color:#fff;border-radius:999px;padding:8px 20px;font-size:13px;font-weight:950}
    .features-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;padding:24px 18px 0}
    .feature-card{background:#fff;border:1px solid var(--line);border-radius:20px;padding:16px;box-shadow:var(--shadow)}
    .feature-icon{font-size:28px;margin-bottom:10px}
    .feature-title{font-size:14px;font-weight:950;margin:0 0 5px;color:#111}
    .feature-desc{font-size:12px;color:#667085;font-weight:700;line-height:1.4;margin:0}
    .section-title{font-size:15px;font-weight:950;margin:24px 18px 12px;color:#111}
    .student-row{display:flex;gap:10px;overflow-x:auto;scrollbar-width:none;padding:0 18px 4px}
    .student-row::-webkit-scrollbar{display:none}
    .student-card{flex-shrink:0;width:100px;text-align:center;cursor:pointer}
    .student-avatar{width:64px;height:64px;border-radius:999px;background:linear-gradient(135deg,var(--blue),var(--cyan));display:flex;align-items:center;justify-content:center;font-size:22px;font-weight:950;color:#fff;margin:0 auto 8px;box-shadow:0 6px 16px rgba(11,108,255,.2)}
    .student-name{font-size:12px;font-weight:850;color:#111;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    .student-school{font-size:11px;color:#667085;font-weight:700}
    .notify-btn{display:block;margin:24px 18px 0;background:linear-gradient(90deg,var(--blue),var(--cyan));color:#fff;border-radius:18px;padding:14px;text-align:center;font-size:15px;font-weight:950;border:0;cursor:pointer;width:calc(100% - 36px)}
...
<div class="app-shell">
  <main class="app-main">
    <div class="topbar">
      ...
      <h1 class="page-heading">Communauté</h1>
    ...
    <div class="coming-hero">
      ...
      <p class="coming-title">Bientôt disponible !</p>
      <p class="coming-sub">La communauté Studimove arrive prochainement. Retrouve tes amis, rejoins des groupes d'étudiants et partage tes aventures.</p>
      <span class="coming-badge">✨ En cours de développement</span>
    </div>

    <div class="features-grid">
      <div class="feature-card"><div class="feature-icon">💬</div><p class="feature-title">Groupes</p><p class="feature-desc">Rejoins des groupes par école ou centre d'intérêt</p></div>
      <div class="feature-card"><div class="feature-icon">📸</div><p class="feature-title">Partage</p><p class="feature-desc">Partage tes photos et souvenirs d'aventures</p></div>
      <div class="feature-card"><div class="feature-icon">🤝</div><p class="feature-title">Rencontres</p><p class="feature-desc">Trouve des co-voyageurs pour tes prochains trips</p></div>
      <div class="feature-card"><div class="feature-icon">🏆</div><p class="feature-title">Classements</p><p class="feature-desc">Compare tes XP avec les autres étudiants</p></div>
    </div>

    <p class="section-title">Déjà inscrits</p>
    <div class="student-row">
      <div class="student-card" onclick="showToast('Profil bientôt disponible')"><div class="student-avatar">L</div><p class="student-name">Léa M.</p><p class="student-school">Sciences Po</p></div>
      <div class="student-card" onclick="showToast('Profil bientôt disponible')"><div class="student-avatar" style="background:linear-gradient(135deg,#FF6B6B,#FFD000)">T</div><p class="student-name">Thomas R.</p><p class="student-school">HEC Paris</p></div>
      <div class="student-card" onclick="showToast('Profil bientôt disponible')"><div class="student-avatar" style="background:linear-gradient(135deg,#00D4FF,#0B6CFF)">C</div><p class="student-name">Camille B.</p><p class="student-school">ESSEC</p></div>
      <div class="student-card" onclick="showToast('Profil bientôt disponible')"><div class="student-avatar" style="background:linear-gradient(135deg,#FF6B6B,#C026D3)">A</div><p class="student-name">Antoine K.</p><p class="student-school">Dauphine</p></div>
      <div class="student-card" onclick="showToast('Profil bientôt disponible')"><div class="student-avatar" style="background:linear-gradient(135deg,#10B981,#0B6CFF)">M</div><p class="student-name">Marie L.</p><p class="student-school">ESCP</p></div>
    </div>

    <button class="notify-btn" onclick="showToast('Tu seras notifié(e) au lancement !')">Me notifier au lancement</button>
  </main>
...
<aside class="drawer" id="drawer" aria-hidden="true">
  ...
  <div class="drawer-menu">
    ...
    <a class="drawer-link active" href="community.php"><span class="menu-ico">👥</span>Communauté</a>
